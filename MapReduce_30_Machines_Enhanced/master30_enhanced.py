...
def split_input_file():
    df = pd.read_csv('/tmp/hmichel-20/sante_publique.txt', sep="\n", header=None)
    nb_line = df.shape[0]
...
def print_result():
    ...
    for file in result_file_list:
        df_file = pd.read_csv(reduce_path + "/" + file)
        df_list_reduced.append(df_file)
    
    df_result = pd.concat(df_list_reduced)
    df_result.to_csv(output_path + "/WC_result.txt", header=None, index=None, sep=',', mode='w')
...
start_map("python3 /tmp/hmichel-20/slave_mapper30_Enhanced.py")
print(("MAPPING --- %s seconds ---\n" % (time.time() - start_time1)))

start_time2 = time.time()
start_shuffle("python3 /tmp/hmichel-20/slave_shuffler30_Enhanced.py")
print(("SHUFFLING --- %s seconds ---\n" % (time.time() - start_time2)))

start_time3 = time.time()
start_reduce("python3 /tmp/hmichel-20/slave_reducer30_Enhanced.py")
